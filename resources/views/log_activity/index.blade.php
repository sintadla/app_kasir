...
<x-content :title="['name'=>'Log Activity', 'icon'=>'fas fa-shoe-prints']">
            ...
            <div class="col">
                @can('admin')
                    <button type="button" class="btn btn-sm btn-danger btn-delete" data-url="{{ route('log.clear') }}">
                        <i class="fas fa-trash mr-2"></i> Bersihkan Semua Log Aktivitas
                    </button>
                @endcan
            </div>
            <div class="col">
                ...
                @forelse ($data as $row)
                    <tr>
                        ...
                        <td>
                            ...
                            <span class="text-muted">({{ ucwords($row->user->username) }})</span>
                            {{ $row->subject }} <br>
                            <small>
                                <span class="text-muted">Tanggal :</span>
                                {{ date('d FY H:i:s', strtotime($row->tanggal)) }}
                            </small> <br>
                        </td>
                        <td>{{ $row->ip }}</td>
